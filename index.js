...
console.log(arr.join('-'));//"-5-103-16-33-344-4
...
function getAverageGrade(student, course) {
   ...
   average= sum/count;
    ...
    if(student[indexof(s)] === student && c.transcript.course === course){
          sum+= s.transcript.grades[i];
           if(i === grades.len){
             count =i;
           }
        }  
...
function getAssignmentMark(student, course, num) {
  ...
  for (let i = 0; i < student.transcript.length; i++) {
      let courseRecord = student.transcript[i];
      if (courseRecord.courseCode === course) {
          if (num < 1 || num > courseRecord.assignments.length) {
              return -1; 
          }
          return courseRecord.assignments[num - 1];
      }
  }
  return -1;
}
...
function averageAssessment(students, courseName, assignment) {
  ...
  for (let i = 0; i < students.length; i++) {
      ...
      for (let j = 0; j < student.transcript.length; j++) {
          let course = student.transcript[j];

          if (course.courseCode === courseName) {
             
              if (assignment >= 1 && assignment <= course.assignments.length) {
                  totalMarks += course.assignments[assignment - 1]; // Adjust for 0-based index
                  count++;
              }
              break; 
          }
  ...
  // Return -1 if no valid marks were found
